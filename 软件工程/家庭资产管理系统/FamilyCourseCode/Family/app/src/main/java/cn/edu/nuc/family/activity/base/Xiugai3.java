...
public class Xiugai3 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xiugai3);
		...
		button1.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				new DatePickerDialog(Xiugai3.this, new DatePickerDialog.OnDateSetListener() {
		...
		if(InputChangeActivity.tableName.equals("������룺")){
			ContentValues cv = new ContentValues();
			SQLiteDatabase UserRead = date_Financialincome.getReadableDatabase();
			Cursor c = UserRead.query("FINANCIALINCOME", null, "FINANCIALINCOME_P=? and FINANCIALINCOME_NO=?", new String[]{PaymentsManagementActivity.no, InputChangeActivity.number} , null, null, null);
			while(c.moveToNext()){
			...
			Cursor c = UserRead.query("FINANCIALOUTCOME", null, "FINANCIALOUTCOME_P=? and FINANCIALOUTCOME_NO=?", new String[]{PaymentsManagementActivity.no, InputChangeActivity.number} , null, null, null);
			while(c.moveToNext()){
				editText1.setText(c.getString(1));
				editText2.setText(c.getString(2));
				editText3.setText(c.getString(3));
		...
		quxiao.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(Xiugai3.this, InputInformationActivity.class);
			...
			public void onClick(View v) {
				if(shouru.isChecked()){
					if(editText1.getText().toString().equals("")||
						...
						Toast.makeText(Xiugai3.this, "������������Ϣ", Toast.LENGTH_SHORT).show();;
					}
					else{
...
//					SQLiteDatabase UserRead = date_Financialincome.getReadableDatabase();
					...
					List.update("LIST", cv2,"LIST_NO=? and LIST_XUHAO=? and LIST_TYPE=?",new String[]{PaymentsManagementActivity.no, InputChangeActivity.number,"������룺"});
					UserWrite.update("FINANCIALINCOME", cv, "FINANCIALINCOME_P=? and FINANCIALINCOME_NO=?", new String[]{PaymentsManagementActivity.no, InputChangeActivity.number});
					UserWrite.close();
					Toast.makeText(Xiugai3.this,"�޸ĳɹ�", Toast.LENGTH_SHORT).show();
...
//					SQLiteDatabase UserRead = date_Financialoutcome.getReadableDatabase();
					...
					List.update("LIST", cv2,"LIST_NO=? and LIST_XUHAO=? and LIST_TYPE=?",new String[]{PaymentsManagementActivity.no, InputChangeActivity.number,"���֧����"});
					UserWrite.update("FINANCIALOUTCOME", cv, "FINANCIALOUTCOME_P=? and FINANCIALOUTCOME_NO=?", new String[]{PaymentsManagementActivity.no, InputChangeActivity.number});
					UserWrite.close();
					Toast.makeText(Xiugai3.this,"�޸ĳɹ�", Toast.LENGTH_SHORT).show();
